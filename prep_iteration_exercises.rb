...
def subwords(word, dictionary)
  valid_substrings = []
  substrings(word).each { |substring| valid_substrings << substring if dictionary.include?(substring) }
  valid_substrings 
end
...
class Array
  ...
  def my_inject(&blk)
    injected = 0
    self.my_each { |ele| injected += blk.call(ele) }
    injected
  end
end
